...
\begin{document}
  ...
  Perhaps this idea can be extended further with explicit negative feedback: what if we partitioned the lexicon entries into two sets: ``word parts'' and ``full words''. Additionally, we give each lexicon entry two probabilities: a probability for its occurrence in the lexicon, and a probability for its occurrence in the corpus.  After calculating the soft counts for a word, we decide whether it is a ``word part'' or a ``full word''. If we classify it as a ``word part'', we artificially decrease its soft count in the corpus, lowering its corpus probability. Similarly, if we classify it as a ``full word'', we artificially decrease its soft count in the lexicon, lowering its lexicon probability. In this way, words and words parts repel each other. 

  It would be interesting to see how this variation performed.

	To automatically learn some word structure at well, each word's lexicon probability could be split into three probabilities: the probability this word occurs as a prefix, the probability this word occurs in the middle of a word, and the probability this word occurs as a suffix. This would encourage words to look like each other structurally and discourage fusing two full words. As above, a similar repelling scheme could be devised to encourage word parts to act as either a prefix, internal fragment, or suffix.
  ...
  Considering the tool we produced, even though we performed lots of internal optimizations, a full run still takes around 5 hours. Optimization is still desired and needed. 
	...
	Although we managed to marginally improve segmentation performance, the corpus found word precision is below 65\%. There still exists room for exciting improvements.
